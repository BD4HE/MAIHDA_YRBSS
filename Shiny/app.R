...
load("./PredictedProbabilities.RData")

df$Stratum=paste0(df$Race,":",df$Sexual_Orientation,":",df$Gender)
df$Stratum=gsub("Multi-Race","MultiRace",df$Stratum)
df$Stratum=gsub("Other Race","OtherRace",df$Stratum)
df$Stratum=gsub("sexual","",df$Stratum)
df$Stratum=gsub("Other-Questioning","OtherSexuality",df$Stratum)
df$Stratum=gsub("Minority (Not Hetero)","SexMinority",df$Stratum)
df$Stratum=gsub("Minority (Not White)","MinorityRace",df$Stratum)
...
Details=list("<b> Predicted probabilities estimated using <a href='https://doi.org/10.1016/j.ssmph.2024.101664'>I-MAIHDA</a> (intersectional multilevel analysis of individual heterogeneity and discriminatory accuracy).</b>",   
             "  
             ",
             "<b>A)</b> 2020 LGBTQ policy tally from <b><a href='https://www.lgbtmap.org/'>MAP</b>.</i>")

names(Details)=c(1,2,3)

Foot2="<b>B)</b> 2020 Negative Racial sentiment Quartiles from <b><a href='https://doi.org/10.1097/EDE.0000000000001671'>BD4HE</b>.</i>"
...
RaceColor=data.frame(Race=c("White","Asian","Black","Hispanic","Multi-Race","Other Race","Minority (Not White)"),Color=c("grey2","red4","darkgreen","skyblue2","orange3","gold3","purple4"))
...
ui <- fluidPage(
  ...
  page_sidebar(
    ...
    sidebar = sidebar(
      selectInput( 
        "Outcome", 
        "Select Mental Health Outcome/Exposure:", 
      ...
      selectInput( 
        "division", 
        "Select Geographic Factor:", 
        choices = unique(df$Geographic),
        selected = "4 Census Regions"
      ),
      
      ############################
      # # well panel for allowing selection of levels
      # wellPanel(
      #   uiOutput("ui")
      # ),
...
server <- function(input, output, session) {
  ...
  # ############################
  # # Render UI for geographic levels if appropriate
  # 
  # observe({
  #   if (length( unique(df$state[df$Geographic==input$division]) ) > 1){
  # output$ui <- renderUI({
  #   selectInput(
  #     "levels", "Select Levels:",
  #     choices = unique(df$state[df$Geographic==input$division]), 
  #     selected = unique(df$state[df$Geographic==input$division]),
  #     multiple = TRUE
  #   )
  # })
  # } else { output$ui <- renderText(unique(df$state[df$Geographic==input$division]))}
  # })
  ...
  ############################
  # Render Table
  output$table <- renderDataTable({datatable(subTable())})
  ...
  subsetted <- reactive({
    ...
    # if (length( unique(df$state[df$Geographic==input$division]) ) > 1){
    #   df <- df |> filter(state %in% input$levels)
  ...
  linely <- observe({
    ...
    output$linely <-renderPlotly({
      ...
      p <- ggplot(subsetted(), aes( x=eval(parse(text=Vars$X)) , y=predicted, shape=Gender, color=eval(parse(text=Vars$ColorVar)),group=Stratum )) +
        theme_light() +
        facet_grid(vars( eval(parse(text=Vars$RowVar)) ), vars( eval(parse(text=Vars$ColumnVar)) )) +
        theme(strip.text.y = element_text(size = 10,face="bold",color="black")) +
        theme(strip.text.x = element_text(size = 10,face="bold",color="black")) +
      ...
      ggplotly(p,tooltip = c("Stratum","predicted","conf.low","conf.high")) %>% 
